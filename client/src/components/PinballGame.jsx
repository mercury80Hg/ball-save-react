...
function PinballGame({ onHighScoreUpdate, onServerReady }) {
  ...
  const initializeGame = () => {
    ...
    style.textContent = getPinballStyles();
    document.head.appendChild(style);

    // Initialize the pinball game using the provided code
    initializePinballGame();
  };

  const initializePinballGame = () => {
    // Matter.js game initialization (simplified version of your provided code)
    const PATHS = {
    ...
    if (window.Matter) {
      ...
      function createStaticBodies() {
        Matter.World.add(world, [
          ...
          createPath(239, 86, PATHS.DOME, COLOR.OUTER),
          createWall(140, 140, 20, 40, COLOR.INNER),
          createWall(225, 140, 20, 40, COLOR.INNER),
          createWall(310, 140, 20, 40, COLOR.INNER),
          createBumper(105, 250),
          createBumper(225, 250),
          createBumper(345, 250),
          createBumper(165, 340),
          createBumper(285, 340),
          createWall(440, 520, 20, 560, COLOR.OUTER),
          createPath(25, 360, PATHS.DROP_LEFT, COLOR.OUTER),
          createPath(425, 360, PATHS.DROP_RIGHT, COLOR.OUTER),
          createWall(120, 510, 20, 120, COLOR.INNER),
          createWall(330, 510, 20, 120, COLOR.INNER),
          createWall(60, 529, 20, 160, COLOR.INNER),
          createWall(390, 529, 20, 160, COLOR.INNER),
          createWall(93, 624, 20, 98, COLOR.INNER, -0.96),
          createWall(357, 624, 20, 98, COLOR.INNER, 0.96),
          createPath(79, 740, PATHS.APRON_LEFT, COLOR.OUTER),
          createPath(371, 740, PATHS.APRON_RIGHT, COLOR.OUTER),
          createReset(225, 50),
          createReset(465, 30),
        ]);
      }
      ...
      function createPath(x, y, path, color) {
        ...
        return Matter.Bodies.fromVertices(x, y, vertices, {
          isStatic: true,
          render: {
            fillStyle: color || COLOR.OUTER,
            strokeStyle: color || COLOR.OUTER,
            lineWidth: 1,
          },
        });
      }
  ...
  const getPinballStyles = () => `
    ...
    @media (min-height: 0px) {
      .pinball-container { transform: scale(0.3); }
    }
    @media (min-height: 400px) {
      .pinball-container { transform: scale(0.4); }
    }
    @media (min-height: 600px) {
      .pinball-container { transform: scale(0.6); }
    }
    @media (min-height: 800px) {
      .pinball-container { transform: scale(0.8); }
    }
  `;
  ...
  return (
    <div className='pinball-game-wrapper'>
      {!gameStarted ? (
        ...
        >
          <h3 style={{ marginBottom: '20px' }}>Pinball Mini-Game</h3>
          <p style={{ textAlign: 'center', marginBottom: '20px' }}>
            Play while the server warms up!
          </p>
          <button
            onClick={startGame}
            style={{
              ...
              boxShadow: '0px 5px 10px 0px rgb(48, 48, 48)',
            }}
          >
            Start Game
          </button>
